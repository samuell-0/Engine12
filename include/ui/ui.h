...
inline void draw_docking_window()
{
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.1227450980392, 0.169803921569, 0.189411764706, 1.0f));
    auto pos = ImGui::GetWindowPos();
    auto size = ImGui::GetWindowSize();
    ImGui::SetNextWindowPos(ImVec2(100, 100));
    ImGui::SetNextWindowSize(ImVec2(100, 100));
    ImGui::Begin("##DockingWindow", nullptr, ImGuiWindowFlags_DockNodeHost | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration);
    ImGui::End();
    ImGui::PopStyleColor();
}
...
{
    if (appstate->ui_data.which_setting != SettingOpened::None) return;
            ...
            if (fullscreen)
            {
                if (ImGui::Button(ICON_FA_COMPRESS "##compress", ImVec2(25.0f, 25.0f)))
                {
                    if (!SDL_SetWindowFullscreen(appstate->window, false))
                    {
                        ...
                        fullscreen = false;
                        ImGui::SetWindowSize("##ViewWindow", ImVec2(0, 100));
...
{
    ImGui::SetNextWindowSizeConstraints(ImVec2(3.0f, 3.0f), ImVec2(appstate->ui_data.window_width, appstate->ui_data.window_hight - 25.0f));
    ...
    // appstate->ui_data.view_weight = ImGui::GetWindowHeight() / (appstate->ui_data.window_hight - 25.0f);
...
inline void draw_buttom_panel(AppState* appstate)
{
    ...
    if (ImGui::BeginTable("##BottomTable", 2, ImGuiTableFlags_Resizable))
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.1227450980392, 0.169803921569, 0.189411764706, 1.0f));
...
inline void draw_visualization_panel(AppState* appstate)
{
    ...
    if (ImGui::BeginTabBar("##DockingTabBar"))
    {
        ...
        if (ImGui::BeginTabItem("docking"))
        {
            if (!ImGui::IsMouseDragging(ImGuiMouseButton_Left))
            {
                ...
                auto pos = ImGui::GetWindowPos();
                auto size = ImGui::GetWindowSize();
                ImGui::SetNextWindowPos(ImVec2(pos.x, pos.y + 20));
                ImGui::SetNextWindowSize(ImVec2(size.x, size.y - 22));
                ImGui::Begin("##DockingWindow", nullptr, ImGuiWindowFlags_DockNodeHost | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration);
                ImGui::DockSpace(1);
                ImGui::End();
                ImGui::PopStyleColor();
